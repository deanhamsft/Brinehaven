...
def create_hidden_root_on_display(display_index, pygame_screen_info=None):

    root = tk.Tk()
    root.overrideredirect(True)          # remove title bar / borders
    root.attributes('-alpha', 0.01)      # almost invisible (0.0 = fully transparent on some OS)
    root.attributes('-topmost', True)    # force on top temporarily

    try:
        # Use pygame to get monitor sizes (assuming primary at 0,0; adjust if your layout differs)
        pygame.init()
        sizes = pygame.display.get_desktop_sizes()
        pygame.quit()
        if display_index < len(sizes):
            mon_w, mon_h = sizes[display_index]
            # Center a small 1x1 or 10x10 window
            x = mon_w // 2 - 5 + (sum(s[0] for s in sizes[:display_index]) if display_index > 0 else 0)
            y = mon_h // 2 - 5
    ...
    root.update()           # apply geometry
    root.focus_force()      # try to force focus (helps Windows/macOS)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_f:
                    root = create_hidden_root_on_display(display_index)  # display_index is the control one
...
if __name__ == "__main__":
    ...
    root = create_hidden_root_on_display(args.c)   # or args.control
